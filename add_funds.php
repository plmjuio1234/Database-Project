...
if (!isset($_SESSION['userid'])) {
    header("Location: login.php"); // 로그인하지 않은 사용자는 로그인 페이지로 리디렉션
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_funds'])) {
    ...
    if ($stmt->affected_rows > 0) {
        echo "<p>Funds added successfully!</p>";
    } else {
        echo "<p>Error adding funds.</p>";
    }
...
<head>
    ...
    document.getElementById('addButton').disabled = true; // 중복 클릭 방지
    $.ajax({
        url: 'add_funds_action.php',
        type: 'POST',
        ...
        success: function(data) {
            if (data.newBalance) {
                ...
                document.getElementById('balance').innerHTML = data.newBalance; // 잔액 업데이트
            } else if (data.error) {
                alert(data.error);
            }
            document.getElementById('addButton').disabled = false; // 버튼 활성화
        },
        error: function(xhr, status, error) {
            alert("Error adding funds: " + error);
            document.getElementById('addButton').disabled = false; // 버튼 활성화
        }
    });
}
                ...
                <!-- <div class="content"> -->
                    <div class="productcontent">
                        ...
                        <div class="html container-xl">
                            ...
                            <input class="btn btn-primary" style="float: right; max-height: 35px;" type="submit" name="add_funds" value="충전하기">
                                <div class="producttitle">원하는 금액 충전하기</div>
                                <div style="margin: 10px;"></div>
                                <div class="line"></div>
                                <div style="margin: 10px;"></div>
                                <div class="container-xl">
                                    <input class="swal2-input col" type="number" name="amount" min="1" placeholder="원하는 금액" id="code">
                                </div>
                            </div>
                        </div>
                    </div>
